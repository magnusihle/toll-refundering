...
import {
  ...
  entryColumns,
  type DetailStrip,
} from "@/components/table/RowDetail";
...
const CONF_VARIANT: Record<string, any> = {
  ...
  raak_grant: "warning",
  no_basis: "outline",
  reclass_strong: "success",
  reclass_possible: "warning",
  reclass_weak: "outline",
};
const confVariant = (v: any): any => CONF_VARIANT[v] ?? "secondary";
const likVariant = (v: any) =>
  v === "høy"
    ? "success"
    : v === "middels"
      ? "warning"
...
export function Recovery() {
  ...
  const kind = TYPES.includes(params.get("type") || "")
    ? params.get("type")!
    : "alle";
  ...
  const setParam = (key: string, value: string | null) => {
    ...
    if (value == null || value === "" || value === "alle" || value === "krav")
      next.delete(key);
  ...
  const baseRows = React.useMemo(
    () =>
      urgentOnly
        ? act.rows.filter(
            (r: any) => r.dager_igjen != null && r.dager_igjen <= 90,
          )
        : act.rows,
    [act.rows, urgentOnly],
  );
  ...
  const byType = React.useMemo(
    () =>
      Object.fromEntries(TYPES.map((k) => [k, agg(rowsFor(materialRows, k))])),
    [materialRows],
  );
  ...
  const claimById = React.useMemo(
    () => new Map(act.rows.map((r: any) => [r.id, r])),
    [act.rows],
  );
  ...
  const visibleIds = React.useMemo(
    () => new Set(rows.map((r: any) => r.id)),
    [rows],
  );
  const selectedOutside = selectedClaims.filter(
    (r) => !visibleIds.has(r.id),
  ).length;
  const selectedAmount = selectedClaims.reduce(
    (sum, r) => sum + (r.amount_nok || 0),
    0,
  );
  ...
  React.useEffect(() => {
    getSent()
      .then((d) => setSentLog(d.items ?? []))
      .catch(() => {
        try {
          setSentLog(JSON.parse(localStorage.getItem(SENT_KEY) || "[]"));
        } catch {}
      });
  }, []);
  ...
  const claimGroupCols = [
    expandColumn(),
    {
      accessorKey: "produkt",
      header: "Produkt",
      cell: (c: any) => <Primary title={c.getValue()}>{c.getValue()}</Primary>,
    },
    {
      accessorKey: "aktor",
      header: "Aktør",
      cell: (c: any) => <Secondary>{c.getValue()}</Secondary>,
    },
    {
      accessorKey: "kind",
      header: "Type",
      cell: (c: any) => <KindBadge kind={c.getValue()} />,
    },
    {
      id: "omfang",
      header: "Fortollinger",
      accessorFn: (r: ClaimGroup) => r.tollnummers.length,
      cell: (c: any) => {
        const g = c.row.original as ClaimGroup;
        return (
          <CountCell value={g.tollnummers.length} of={g.count} ofLabel="krav" />
        );
      },
    },
    {
      id: "frist",
      header: "Første frist",
      accessorFn: (r: ClaimGroup) => r.dager_igjen,
      cell: (c: any) => {
        const g = c.row.original as ClaimGroup;
        return (
          <Deadline
            days={g.dager_igjen}
            note={g.urgentCount > 1 ? `(${g.urgentCount} haster)` : undefined}
          />
        );
      },
    },
    {
      ...
      header: "Beløp",
      cell: (c: any) => <MoneyCell nok={c.getValue()} />,
    },
    {
      id: "match",
      header: "Match",
      accessorFn: (r: ClaimGroup) => r.confidences.join(" "),
      cell: (c: any) => (
        <MultiValue
          values={(c.row.original as ClaimGroup).confidences}
          render={(v) => <Badge variant={confVariant(v)}>{confLabel(v)}</Badge>}
        />
      ),
    },
    ...(hasAgent
      ? [
          {
            id: "lik",
            header: "Gjenv.",
            accessorFn: (r: ClaimGroup) => r.likelihoods.join(" "),
            cell: (c: any) => (
              <MultiValue
                values={(c.row.original as ClaimGroup).likelihoods}
                render={(v) => <Badge variant={likVariant(v)}>{v}</Badge>}
              />
            ),
          },
        ]
  ...
  const claimFlatCols = [
    ...
    {
      accessorKey: "tollnummer",
      header: "Tollnummer",
      cell: (c: any) => <Code>{c.getValue()}</Code>,
    },
    {
      id: "frist",
      header: "Frist",
      accessorFn: (r: any) => r.dager_igjen,
      cell: (c: any) => <Deadline days={c.getValue()} />,
    },
    {
      accessorKey: "amount_nok",
      header: "Beløp",
    ...
    },
    {
      ...
      header: "Match",
      cell: (c: any) => (
        <Badge variant={confVariant(c.getValue())}>
          {confLabel(c.getValue())}
        </Badge>
      ),
    },
    ...(hasAgent
      ? [
          {
            ...
            cell: (c: any) =>
              c.getValue() ? (
                <Badge variant={likVariant(c.getValue())}>{c.getValue()}</Badge>
              ) : (
  ...
  const bkuBody = (bku: any) => (
    <span className="block space-y-1.5">
            ...
            {list.map((e: any, i: number) => (
              <React.Fragment key={e.id}>
                ...
                <a
                  ...
                  title={`${e.id} · ${e.publishDate ?? ""} · ${e.description}`}
                >
                  {e.itemType || e.id}
                </a>
                {e.binding ? null : (
                  <span className="text-2xs uppercase"> (CO)</span>
                )}
              </React.Fragment>
            ))}
          </span>
      ...
      <span className="block text-xs">
        Uttalelsene gjelder varene de er avgitt for — presedens for denne varen,
        ikke et vedtak om den.
      </span>
    </span>
  );
  ...
  const normalise = (t?: string | null) =>
    (t ?? "")
      ...
      .replace(/[-\s]+/g, " ")
      .replace(
        /\b(a\/s|as|asa|aps|ab|oy|oyj|gmbh|ltd|inc|bv|nv|srl|sa|kg)\b\.?/gi,
        "",
      )
      .replace(/\s{2,}/g, " ")
      .trim();
  ...
  const distinctTexts = (
    claims: any[],
    field: string,
  ): { text: string; count: number; amount: number }[] => {
    const byKey = new Map<
      string,
      { text: string; count: number; amount: number }
    >();
    for (const c of claims) {
      ...
      if (
        ...
        (c.amount_nok ?? 0) > 0 &&
        raw.length &&
        (c.amount_nok ?? 0) > hit.amount / hit.count
      )
        hit.text = raw;
    }
  ...
  const fmtAmount = (v: number, partial: boolean) =>
    partial && v > 0 ? `${n(Math.round(v))} kr` : null;
  ...
  const claimStrips = (
    ...
    return [
      ...(lead
        ? [{ label: "Hva det er", tone: "info", body: lead } as DetailStrip]
        : []),
      ...(r.reasoning
        ? [
            {
              label: "Agent-vurdering",
              tone: "info",
              values: r.likelihood
                ? [`${r.likelihood} sannsynlighet`]
                : undefined,
              body: r.reasoning,
            } as DetailStrip,
          ]
      ...
      ...(r.claim_draft
        ? [
            {
              label: "Utkast til krav",
              tone: "action",
              values: [opts.scope, deadlineOnly || null].filter(
                Boolean,
              ) as string[],
              body: r.claim_draft,
            } as DetailStrip,
          ...
          ? [
              {
                label: "Neste steg",
                tone: "action",
                values: opts.scope ? [opts.scope] : undefined,
                body: r.action,
              } as DetailStrip,
            ]
          : []),
      ...(r.bku &&
      (r.bku.proposed?.length ?? 0) + (r.bku.declared?.length ?? 0) > 0
        ? [
            {
              label: "BKU-presedens",
              tone: "info",
  ...
  const claimSourceColumns = () =>
    entryColumns(
      [
        ...
        COL.node("Match", (r: any) => (
          <Badge variant={confVariant(r.confidence)}>
  ...
  const claimFlatDetail = (row: any) => {
    ...
    return (
      <RowDetail
        ...
        source={{
          caption: "Fortollingen kravet gjelder",
  ...
  const claimGroupDetail = (row: any) => {
    ...
    const strips: DetailStrip[] = [
      ...(leads
        .map((x) => {
          const lead = summaryLead(
            x.text,
            reasons.find((r) => x.text.includes(r.text))?.text,
          );
          return lead
            ? ({
                label: "Hva det er",
                tone: "info",
                values: [
                  scopeLabel(x.count, total),
                  fmtAmount(x.amount, x.count < total),
      ...
      ...reasons.map(
        (x) =>
          ({
            ...
            values: [
              g.shared.likelihood
                ? `${g.shared.likelihood} sannsynlighet`
                : null,
              scopeLabel(x.count, total),
              fmtAmount(x.amount, x.count < total),
            ].filter(Boolean) as string[],
      ...
      ...(g.shared.bku &&
      (g.shared.bku.proposed?.length ?? 0) +
        (g.shared.bku.declared?.length ?? 0) >
        0
        ? [
            {
              label: "BKU-presedens",
  ...
  const dismissedCols = [
    ...
    {
      accessorKey: "betalt_toll",
      header: "Betalt toll",
      cell: (c: any) => <MoneyCell nok={c.getValue()} tone="muted" />,
    },
    {
      id: "status",
      header: "Vurdering",
      cell: () => <Badge variant="outline">ikke grunnlag</Badge>,
    },
  ];
  ...
  const raakCols = [
    ...
    {
      accessorKey: "godkjent",
      header: "Fortollet",
      cell: (c: any) => <Code>{c.getValue()}</Code>,
    },
    {
      id: "gyldig",
      header: "Vedtak gyldig",
      cell: ({ row }: any) => (
        <span className="whitespace-nowrap text-xs tabnum text-muted-foreground">
          {row.original.granted_fom || "—"} → {row.original.granted_tom || "—"}
        </span>
      ),
    },
    {
      ...
      header: "Betalt sats",
      cell: (c: any) => <Code>{c.getValue()} kr/kg</Code>,
    },
    {
      accessorKey: "raak_amount",
      header: "RÅK betalt",
      cell: (c: any) => <MoneyCell nok={c.getValue()} tone="muted" />,
    },
    {
    ...
    () => [
      {
        ...
        options: TYPES.map((k) => ({
          value: k,
          label: k === "alle" ? "Alle typer" : k,
          count: (byType[k] as any).count,
        })),
      ...
      },
      {
        ...
        options: MIN_AMOUNT_OPTIONS.map((v) => ({
          value: String(v),
          label: v === 0 ? "Alle beløp" : `≥ ${n(v)} kr`,
        })),
        apply: (list, v) => splitByAmount(list, Number(v)).material,
        explain: (
          <>
            Hver fortolling må omberegnes for seg i TVINN. Under grensen koster
            et krav mer å hente enn det gir. Ingenting slettes — antall og beløp
            står alltid under tabellen.
          </>
        ),
      },
    ],
  ...
  const view = {
    ...
    options: [
      { value: "grouped", label: "Gruppert" },
      { value: "flat", label: "Alle krav" },
    ],
  };
      ...
      <div className="rule-t flex flex-wrap items-center gap-x-3 gap-y-2 pt-4">
        <span className="t-small">
          <span className="tabnum font-medium text-foreground">
            {n(selectedClaims.length)}
          </span>{" "}
          valgt
          {" · "}
          <span className="tabnum">
            <Amount nok={selectedAmount} />
          </span>
          {selectedOutside > 0 && (
            <span className="text-muted-foreground">
              {" "}
              · {n(selectedOutside)} utenfor gjeldende filter
            </span>
          )}
        </span>
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          <Button
            variant="outline"
            onClick={() =>
              exportXlsx(selectedClaims, groupClaims(selectedClaims), fileName)
            }
          >
            <Download />
            Last ned Excel
          </Button>
          <Button onClick={prepareEmail}>
            <Mail />
            Send til 3PL
          </Button>
        </div>
    ...
    <>
      ...
      <StatRow cols={3}>
        <StatCard
          ...
          hint={
            <>
              {n(a.count)} krav, vektet. Øvre tak <Amount nok={a.ceiling} />.
            </>
          }
        />
        <StatCard
          label="Solid grunnlag"
          icon={ShieldCheck}
          value={<Amount nok={a.solid} />}
          hint={
            <>
              {n(a.assessed)} av {n(a.count)} agent-vurdert mot tolltariffen.
            </>
          }
        />
        <StatCard
      ...
      <TableSection
        ...
        footer={
          lastSent ? (
            <span
              title={`Utvalg: ${lastSent.filter || "alle"}${lastSent.sender ? " · av " + lastSent.sender : ""}`}
            >
              Sist sendt{" "}
              {new Date(lastSent.at).toLocaleDateString("nb-NO", {
                day: "numeric",
                month: "short",
              })}
              : {n(lastSent.count)} krav (est. {n(lastSent.amount)} kr) —
              avventer svar fra 3PL.
            </span>
          ) : undefined
        }
      >
        {mode === "grouped" ? (
          <DataTable
            columns={claimGroupCols}
            data={groups}
            filterPlaceholder="Søk produkt / aktør…"
            initialFilter={q}
            defs={defs}
            filters={filters}
            view={view}
            total={act.rows.length}
            unit="krav"
            selection={{
              ids: selectedIds,
              idsOf: (g: ClaimGroup) => g.claims.map((c: any) => c.id),
              onToggle: toggleSelection,
              bar: claimActionBar,
            }}
            getRowCanExpand={() => true}
            renderSubComponent={claimGroupDetail}
            empty={
              urgentOnly
                ? "Ingen krav med frist innen 90 dager i dette utvalget."
                : undefined
            }
          />
        ) : (
          <DataTable
            columns={claimFlatCols}
            data={rows}
            ...
            getRowCanExpand={() => true}
            renderSubComponent={claimFlatDetail}
            empty={
              urgentOnly
                ? "Ingen krav med frist innen 90 dager i dette utvalget."
                : undefined
      ...
      >
        <div className="space-y-2">
          <Evidence
            ...
            note={
              <>
                Tollen er korrekt betalt. <Amount nok={dismissed?.ceiling} />{" "}
                ble tidligere vist som mulig refusjon.
              </>
            }
          >
            <DataTable
              columns={dismissedCols}
                  ...
                  strips={[
                    {
                      ...
                      tone: "info",
                      body:
                        row.original.begrunnelse ||
                        "Ingen begrunnelse registrert.",
                    },
                  ]}
                  source={{
                    ...
                    columns: entryColumns(
                      ...
                      {
                        godkjent: (r: any) =>
                          entries.get(r.tollnummer)?.godkjent,
                        deadline: (r: any) =>
                          entries.get(r.tollnummer)?.days_left,
                        sad: (r: any) => entries.get(r.tollnummer)?.sad_url,
                      },
                    ),
                    rows: [row.original],
                ...
                const strips: DetailStrip[] = [
                  {
                    ...
                    values: [
                      `vedtak gyldig ${r.granted_fom || "—"} – ${r.granted_tom || "—"}`,
                    ],
                    body: (
                      <>
                        Match i register: «{(r.matched_product || "").trim()}» (
                        {r.confidence === "strong" ? "sterk" : "svak"}) ·{" "}
                        Standardsats på datoen:{" "}
                        {r.standard_rate_status === "gyldig"
                ...
                return (
                  <RowDetail
                    ...
                    source={{
                      ...
                      columns: entryColumns(
                        [
                          COL.node("Betalt sats", (x: any) => (
                            <Code>{x.applied_rate} kr/kg</Code>
                          )),
                          COL.money("RÅK betalt", (x: any) => x.raak_amount),
                        ],
                        {
                          deadline: (x: any) =>
                            entries.get(x.tollnummer)?.days_left,
      ...
      <Section
        ...
        footer={
          <>
            Datagrunnlag: {n(cov?.n)} deklarasjoner ({cov?.first || "—"} –{" "}
            {cov?.last || "—"}), 3-årsvindu fra {cov?.window?.from}.
            {cov?.beforeWindow
              ? ` ${n(cov.beforeWindow)} eldre er utelatt (foreldet).`
              : ""}
            {unassessed?.count > 0 ? (
              <>
                {" "}
                {n(unassessed.count)} mindre linjer (
                <Amount nok={unassessed.ceiling} /> betalt toll) er ennå ikke
                agent-vurdert og holdes utenfor totalene.
              </>
            ) : null}
          </>
        }
      >
        ...
        <ol className="ml-4 list-decimal space-y-1.5 text-sm text-muted-foreground">
          <li>
            Velg kravene du vil sende, og send dem til 3PL. E-postprogrammet
            åpnes med et kort følgebrev, og Excel-arbeidsboken med begrunnelse
            og kravtekst per fortolling lastes ned samtidig.
          </li>
          <li>
            Fyll inn adressen, legg ved Excel-filen, og send. Følgebrevet ber om{" "}
            <Lift>omberegning i TVINN</Lift> per fortolling.
          </li>
          <li>
            Hva 3PL trenger per kravtype:
            <dl className="mt-1.5 space-y-1">
              <div className="flex gap-2">
                <dt className="min-w-[5.5rem] font-medium text-foreground">
                  Preferanse
                </dt>
                <dd>gyldig opprinnelsesbevis.</dd>
              </div>
              <div className="flex gap-2">
                <dt className="min-w-[5.5rem] font-medium text-foreground">
                  RÅK
                </dt>
                <dd>skrivnummeret står i arket — vedtaket finnes allerede.</dd>
              </div>
              <div className="flex gap-2">
                <dt className="min-w-[5.5rem] font-medium text-foreground">
                  Produkt
                </dt>
                <dd>avklar HS/MVA-sats.</dd>
              </div>
            </dl>
          </li>
          <li>
            <Lift>Fristen er 3 år</Lift> etter fortolling. Hastesakene står
            øverst i arket, med rød frist.
          </li>
          <li>
            Avvent svar: følgebrevet ber 3PL bekrefte mottak, omberegne per
            fortolling, og legge inn preferanse + RÅK-skrivnummer fremover.
          </li>
        </ol>
      </Section>
    </>
